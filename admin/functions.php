...
$action =$_GET['action'];
    ...
    case "insert":{
        ...
        if(isset($_POST["operation"]))
        {
         if($_POST["operation"] == "Add")
         {
          ...
          $statement = $connection->prepare("
           INSERT INTO patient (f_name, l_name,address,contact_no,dob,img,doctor_name) 
           VALUES (:first_name, :last_name, :address,:phno,:dob,:image,:did)
          ");
          $result = $statement->execute(
           array(
  ...
  break;
    ...
    case "fetch":
    {
        ...
        {
         $query = 'SELECT * FROM patient WHERE f_name LIKE "%'.$_POST["search"]["value"].'%" ';
        ...
        //  $sub_array[] ='<form action="dashboard.php"><input type="submit" value="New Booking" /></form>';
 ...
 case "fetch-my-patients":
  {

      $query = '';
      $output = array();
      $query .= "SELECT * FROM patient WHERE doctor_name = '".$_SESSION['username']."'";
      if(isset($_POST["search"]["value"]))
      {
       $query = 'SELECT * FROM patient WHERE f_name LIKE "%'.$_POST["search"]["value"].'%" AND doctor_name ="'.$_SESSION['username'].'"';
      $query .= 'OR address LIKE "%'.$_POST["search"]["value"].'%" AND doctor_name ="'.$_SESSION['username'].'"';
      }
      if(isset($_POST["order"]))
      {
        $query .= 'ORDER BY '.$_POST['order']['0']['column'].' '.$_POST['order']['0']['dir'].' ';
      ...
      foreach($result as $row)
      {
       ...
       $sub_array[] = $row["address"];
       $sub_array[] = $row["contact_no"];
       $sub_array[] ='<form action="dashboard.php" method="post" ><input type="hidden" name="id" value="'.$row["id"].'" /><input type="submit" class="btn btn-success btn-xs " value="Appiontments" /></form>';
